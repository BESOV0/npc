...
--*/
{
   ...
   //printf("PAL_PlayerInfoBox 3\n");
#ifndef PAL_CLASSIC
   //
   // Draw a border for the Time Meter
   //
   //printf("PAL_PlayerInfoBox 4\n");
   rect.x = PAL_X(pos) + 31;
   rect.y = PAL_Y(pos) + 4;
   rect.w = 1;
   rect.h = 6;
   ...
   for (i = 0; i < BATTLEUI_MAX_SHOWNUM; i++)
   {
      if (g_Battle.UI.rgShowNum[i].wNum > 0)
      {
         if ((SDL_GetTicks() - g_Battle.UI.rgShowNum[i].dwTime) / BATTLE_FRAME_TIME > 1000000)
         {
               ...
               printf("clear the num\n");
         }
         else
         {
            PAL_DrawNumber(g_Battle.UI.rgShowNum[i].wNum, 5,
               PAL_XY(PAL_X(g_Battle.UI.rgShowNum[i].pos), PAL_Y(g_Battle.UI.rgShowNum[i].pos) - (SDL_GetTicks() - g_Battle.UI.rgShowNum[i].dwTime)  / BATTLE_FRAME_TIME / 30),
